### 👋 Hi, there! welcome to my GitHub profile! My name is Muchammad Malik, and you can call me Malik 

🔭 Currently, I am working as data engineer at FIFGROUP. Before working at current company, I have completed more than 15+ data projects and graduated from 2 bootcamps (data science and data engineer bootcamps). I'm passionate to learn tech-related skills and explore new technology to solve real-world problems and creating innovative solutions that make an impact to business or society.

🌱 To support my interest at data and analytics field, I've mastered several tools and programming language, such as:
-	Programming language: SQL, Python, bash / shell script, C++
-	Database: Oracle, PostgreSQL, MySQL, BigQuery, MongoDB
-	Business intelligence tools: Tableau, Power BI, Looker Studio
-	Data pipeline tools: Airflow, Pentaho, Oracle Goldengate 
-	Google Cloud Platform : BigQuery, Looker Studio, Dataflow, Dataprep, etc

✍ On my GitHub, you'll find a collection of personal projects and contributions to open-source projects that I've worked on. These range from exploratory data analytics, dashboard creation, and machine learning model development. I'm always looking for new challenges and opportunities to collaborate with others on interesting projects.

💬 If you're interested in working together or have any questions about my work, feel free to reach out. 

😄 Thanks for stopping by, and happy coding!


<!--
**mmalik17/mmalik17** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
